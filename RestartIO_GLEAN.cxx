...
RestartIO_GLEAN::RestartIO_GLEAN() : m_basePathName(0),
                                     m_basePathNameLen(GLEAN_MAX_STRING_LEN),
                                     m_partFileName(0),
                                     m_partFileNameLen(GLEAN_MAX_STRING_LEN),
                                     m_localParticles(-1),
                                     m_localParticles_verify(-1),
                                     m_totPartParticles(-1),
                                     m_totGlobalParticles(-1),
                                     m_mode(UNDEF_MODE),
                                     m_interface(USE_SYNC_MPIIO),
                                     m_header_mode(USE_SYNC_MPIIO_FOR_HEADER),
...
int RestartIO_GLEAN ::CreateCheckpoint(char *pathname, int64_t &num_particles)
{
    ...
    m_localParticles_verify = num_particles;
    ...
    //std::cout << "m_totPartParticles:" << m_totPartParticles << std::endl;
...
int64_t RestartIO_GLEAN ::OpenRestart(char *pathname)
{
 ...
 if (m_interface != USE_ASYNC_MPIIO){
    // verifies content of header. For async, do this later
...
else //trust me for now, but check at end of read phase
    m_localParticles = m_localParticles_verify;
...
int RestartIO_GLEAN ::__MPIIO_Read_Header(void)
{
    ...
    if (0 == m_partitionRank)
    {
        ...
        switch (m_header_mode)
        {
        case USE_SYNC_MPIIO_FOR_HEADER:
            errcode = MPI_File_read_at(m_fileHandle, ofst, m_header, m_headerSize, MPI_BYTE, &status);
            break;
        case USE_ASYNC_MPIIO_FOR_HEADER:
            errcode = MPI_File_iread_at(m_fileHandle, ofst, m_header, m_headerSize, MPI_BYTE, &request_read_header);
            break;
        default:
            errcode = MPI_File_read_at(m_fileHandle, ofst, m_header, m_headerSize, MPI_BYTE, &status);
            break;
        ...
        //if (m_header_mode != USE_ASYNC_MPIIO_FOR_HEADER) //check that async has ended before closing the file (no real async read!)
            //MPI_Wait(&request_read_header, MPI_STATUS_IGNORE);
        if (m_header_mode != USE_ASYNC_MPIIO_FOR_HEADER)
            MPI_File_close(&m_fileHandle);
    }
...
void RestartIO_GLEAN ::Set_Sync_MPI_IO_Interface(void)
{
    m_interface = USE_SYNC_MPIIO;
    return;
}

void RestartIO_GLEAN ::Set_Async_MPI_IO_Interface(void)
{
    m_interface = USE_ASYNC_MPIIO;
    return;
}

void RestartIO_GLEAN ::Set_Async_MPI_IO_Header(void)
{
    m_header_mode = USE_ASYNC_MPIIO_FOR_HEADER;
    return;
}
void RestartIO_GLEAN ::Set_Sync_MPI_IO_Header(void)
{
    m_header_mode = USE_SYNC_MPIIO_FOR_HEADER;
    return;
}

void RestartIO_GLEAN ::Set_POSIX_IO_Interface(int val)
{
    m_interface = USE_POSIX;
...
GLEAN_IO_INTERFACE RestartIO_GLEAN ::Get_IO_Interface(void)
{
    return m_interface;
}
...
void RestartIO_GLEAN::PrintIOCoordInfo(void)
{
    if (0 == m_globalCommRank)
    {
        ...
        case GLEAN_FILE_PER_RANK:
            cout << " File Mode : File Per Rank" << endl;
            break;

#ifdef __bgq__
        case GLEAN_BGQ_ION_PART:
            // BGQ Mechanism
            cout << " File Mode : BGQ File per ION" << endl;
            break;
#endif

        case GLEAN_SINGLE_FILE:
        default:
            cout << " File Mode : Single Shared File" << endl;
            break;
        }
        switch (m_interface)
        {
        ...
        case USE_POSIX:
            cout << " Write Mode: POSIX" << endl;
            cout << " Read Mode : POSIX" << endl;
            break;
        case USE_SYNC_MPIIO:
            cout << " Write Mode: Sync MPI I/O" << endl;
            cout << " Read Mode : Sync MPI I/O" << endl;
            break;
        case USE_ASYNC_MPIIO:
            cout << " Write Mode: Async MPI I/O" << endl;
            cout << " Read Mode : Async MPI I/O" << endl;
            break;
        case UNDEF_INTERFACE:
            cout << " Write Mode: Undefined" << endl;
            cout << " Read Mode : Undefined" << endl;
            break;
        }
...
int RestartIO_GLEAN::__initalize_BGQ_PartitionInfo(void)
{
    ...
    color = (A_color * (B_blocks * C_blocks * D_blocks * E_blocks)) + (B_color * (C_blocks * D_blocks * E_blocks)) + (C_color * (D_blocks * E_blocks)) + (D_color * (E_blocks)) + E_color;
...
int RestartIO_GLEAN ::__MPIIO_Close_Checkpoint(void)
{
    ...
    if (0 == m_partitionRank)
    {
        ...
        if (m_header_mode != USE_ASYNC_MPIIO_FOR_HEADER)
        {
            errcode = MPI_File_write_at(m_fileHandle, ofst, m_header, m_headerSize, MPI_BYTE, &status);

            //error detect and delete
            if (MPI_SUCCESS != errcode)
...
#endif
    if (0 == m_partitionRank && m_header && m_header_mode == USE_ASYNC_MPIIO_FOR_HEADER)
    {
        delete[] m_header;
        m_header = 0;
    }
...
{
#ifdef SCOREP
    SCOREP_USER_REGION_BEGIN(handle3, "write", SCOREP_USER_REGION_TYPE_COMMON)
#endif
    ...
    else if (m_interface == USE_SYNC_MPIIO)
    {
    ...
    else if (m_interface == USE_ASYNC_MPIIO)
    {

        ofst += scan_size * record_size;
        // errcode = MPI_File_iwrite_at(m_fileHandle, ofst, xx, num_particles, MPI_FLOAT, &request_write_xx);
        errcode = MPI_File_iwrite_at(m_fileHandle, ofst, xx, num_particles, MPI_FLOAT, &requests_write[0]);
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iwrite_at(m_fileHandle, ofst, yy, num_particles, MPI_FLOAT, &request_write_yy);
        errcode = MPI_File_iwrite_at(m_fileHandle, ofst, yy, num_particles, MPI_FLOAT, &requests_write[1]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Write_At YY");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iwrite_at(m_fileHandle, ofst, zz, num_particles, MPI_FLOAT, &request_write_zz);
        errcode = MPI_File_iwrite_at(m_fileHandle, ofst, zz, num_particles, MPI_FLOAT, &requests_write[2]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Write_At ZZ");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iwrite_at(m_fileHandle, ofst, vx, num_particles, MPI_FLOAT, &request_write_vx);
        errcode = MPI_File_iwrite_at(m_fileHandle, ofst, vx, num_particles, MPI_FLOAT, &requests_write[3]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Write_At VX");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iwrite_at(m_fileHandle, ofst, vy, num_particles, MPI_FLOAT, &request_write_vy);
        errcode = MPI_File_iwrite_at(m_fileHandle, ofst, vy, num_particles, MPI_FLOAT, &requests_write[4]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Write_At VY");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iwrite_at(m_fileHandle, ofst, vz, num_particles, MPI_FLOAT, &request_write_vz);
        errcode = MPI_File_iwrite_at(m_fileHandle, ofst, vz, num_particles, MPI_FLOAT, &requests_write[5]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Write_At VZ");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iwrite_at(m_fileHandle, ofst, phi, num_particles, MPI_FLOAT, &request_write_phi);
        errcode = MPI_File_iwrite_at(m_fileHandle, ofst, phi, num_particles, MPI_FLOAT, &requests_write[6]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Write_At PHI");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iwrite_at(m_fileHandle, ofst, pid, num_particles, MPI_LONG_LONG, &request_write_pid);
        errcode = MPI_File_iwrite_at(m_fileHandle, ofst, pid, num_particles, MPI_LONG_LONG, &requests_write[7]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Write_At PID");
        }
        ...
        ofst += num_particles * sizeof(int64_t);
        // errcode = MPI_File_iwrite_at(m_fileHandle, ofst, mask, num_particles, MPI_UNSIGNED_SHORT, &request_write_mask);
        errcode = MPI_File_iwrite_at(m_fileHandle, ofst, mask, num_particles, MPI_UNSIGNED_SHORT, &requests_write[8]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Write_At Mask");
        }
    ...
    // Have Root Write the Header Out in Async mode
    if (0 == m_partitionRank && m_header_mode == USE_ASYNC_MPIIO_FOR_HEADER)
                  MPI_File_iwrite_at(m_fileHandle, 0, m_header, m_headerSize, MPI_BYTE, &request_write_header);
...

{
    ...
    {
        ofst += scan_size * record_size;
        // errcode = MPI_File_iread_at(m_fileHandle, ofst, xx, num_particles, MPI_FLOAT, &request_read_xx);
        errcode = MPI_File_iread_at(m_fileHandle, ofst, xx, num_particles, MPI_FLOAT, &requests_read[0]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Read_At XX");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iread_at(m_fileHandle, ofst, yy, num_particles, MPI_FLOAT, &request_read_yy);
        errcode = MPI_File_iread_at(m_fileHandle, ofst, yy, num_particles, MPI_FLOAT, &requests_read[1]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Read_At YY");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iread_at(m_fileHandle, ofst, zz, num_particles, MPI_FLOAT, &request_read_zz);
        errcode = MPI_File_iread_at(m_fileHandle, ofst, zz, num_particles, MPI_FLOAT, &requests_read[2]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Read_At ZZ");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iread_at(m_fileHandle, ofst, vx, num_particles, MPI_FLOAT, &request_read_vx);
        errcode = MPI_File_iread_at(m_fileHandle, ofst, vx, num_particles, MPI_FLOAT, &requests_read[3]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Read_At VX");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iread_at(m_fileHandle, ofst, vy, num_particles, MPI_FLOAT, &request_read_vy);
        errcode = MPI_File_iread_at(m_fileHandle, ofst, vy, num_particles, MPI_FLOAT, &requests_read[4]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Read_At VY");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iread_at(m_fileHandle, ofst, vz, num_particles, MPI_FLOAT, &request_read_vz);
        errcode = MPI_File_iread_at(m_fileHandle, ofst, vz, num_particles, MPI_FLOAT, &requests_read[5]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Read_At VZ");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iread_at(m_fileHandle, ofst, phi, num_particles, MPI_FLOAT, &request_read_phi);
        errcode = MPI_File_iread_at(m_fileHandle, ofst, phi, num_particles, MPI_FLOAT, &requests_read[6]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Read_At PHI");
        }
        ...
        ofst += num_particles * sizeof(float);
        // errcode = MPI_File_iread_at(m_fileHandle, ofst, pid, num_particles, MPI_LONG_LONG, &request_read_pid);
        errcode = MPI_File_iread_at(m_fileHandle, ofst, pid, num_particles, MPI_LONG_LONG, &requests_read[7]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Read_At PID");
        }
        ...
        ofst += num_particles * sizeof(int64_t);
        // errcode = MPI_File_iread_at(m_fileHandle, ofst, mask, num_particles, MPI_UNSIGNED_SHORT, &request_read_mask);
        errcode = MPI_File_iread_at(m_fileHandle, ofst, mask, num_particles, MPI_UNSIGNED_SHORT, &requests_read[8]);
        if (MPI_SUCCESS != errcode)
        {
            __HandleMPIIOError(errcode, (char *)"MPI_FILE_Read_At Mask");
        }
...
// Wait functions 
//----------------------------------
void RestartIO_GLEAN ::__check_read_end(void)
{
    ...
    if (m_interface == USE_ASYNC_MPIIO)
    {
        // MPI_Wait(&request_read_xx, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_read_yy, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_read_zz, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_read_vx, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_read_vy, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_read_vz, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_read_phi, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_read_pid, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_read_mask, MPI_STATUS_IGNORE);
        MPI_Waitall(9, requests_read, MPI_STATUSES_IGNORE);
    }
}

void RestartIO_GLEAN ::__check_write_end(void)
{
    
    if (0 == m_partitionRank && m_header_mode == USE_ASYNC_MPIIO_FOR_HEADER)
        MPI_Wait(&request_write_header, MPI_STATUS_IGNORE);

    if (m_interface == USE_ASYNC_MPIIO)
    {
        // MPI_Wait(&request_write_xx, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_write_yy, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_write_zz, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_write_vx, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_write_vy, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_write_vz, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_write_phi, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_write_pid, MPI_STATUS_IGNORE);
        // MPI_Wait(&request_write_mask, MPI_STATUS_IGNORE);
        MPI_Waitall(9, requests_write, MPI_STATUSES_IGNORE);
    }
}
...
// Test functions 
//----------------------------------
void RestartIO_GLEAN :: test_read(void)
{


    if (0 == m_partitionRank && m_header_mode == USE_ASYNC_MPIIO_FOR_HEADER)
        MPI_Test(&request_read_header,&test_header, MPI_STATUS_IGNORE);

    if (m_interface == USE_ASYNC_MPIIO)
    {
        // MPI_Test(&request_read_xx, &test_xx, MPI_STATUS_IGNORE);
        // MPI_Test(&request_read_yy, &test_yy, MPI_STATUS_IGNORE);
        // MPI_Test(&request_read_zz, &test_zz, MPI_STATUS_IGNORE);
        // MPI_Test(&request_read_vx, &test_vx, MPI_STATUS_IGNORE);
        // MPI_Test(&request_read_vy, &test_vy, MPI_STATUS_IGNORE);
        // MPI_Test(&request_read_vz, &test_vz, MPI_STATUS_IGNORE);
        // MPI_Test(&request_read_phi, &test_phi, MPI_STATUS_IGNORE);
        // MPI_Test(&request_read_pid, &test_pid, MPI_STATUS_IGNORE);
        // MPI_Test(&request_read_mask, &test_mask, MPI_STATUS_IGNORE);
        MPI_Testall(9, requests_read,&test ,MPI_STATUSES_IGNORE);
    }
}

void RestartIO_GLEAN :: test_write(void)
{
    
    if (0 == m_partitionRank && m_header_mode == USE_ASYNC_MPIIO_FOR_HEADER)
        MPI_Test(&request_write_header, &test_header, MPI_STATUS_IGNORE);

    if (m_interface == USE_ASYNC_MPIIO)
    {
        // MPI_Test(&request_write_xx, &test_xx, MPI_STATUS_IGNORE);
        // MPI_Test(&request_write_yy, &test_yy, MPI_STATUS_IGNORE);
        // MPI_Test(&request_write_zz, &test_zz, MPI_STATUS_IGNORE);
        // MPI_Test(&request_write_vx, &test_vx, MPI_STATUS_IGNORE);
        // MPI_Test(&request_write_vy, &test_vy, MPI_STATUS_IGNORE);
        // MPI_Test(&request_write_vz, &test_vz, MPI_STATUS_IGNORE);
        // MPI_Test(&request_write_phi, &test_phi, MPI_STATUS_IGNORE);
        // MPI_Test(&request_write_pid, &test_pid, MPI_STATUS_IGNORE);
        // MPI_Test(&request_write_mask, &test_mask, MPI_STATUS_IGNORE);
        MPI_Testall(9, requests_write,&test ,MPI_STATUSES_IGNORE);
    }
}
...
// validate function
// -----------------
void RestartIO_GLEAN ::__check_valid_read(void){
    //! checks that the header is read correctly
...
if (m_interface == USE_ASYNC_MPIIO){
    ...
    // Check that the same global rank is reading it back up
    assert(g_rank == rank_in_file);
        if (m_localParticles_verify != m_localParticles)
                {
